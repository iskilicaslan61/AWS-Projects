...
def calculate():
    if request.method == "POST":
        ...
        return render_template("result.html", result1 = num1, result2 = num2, lcm = lcm(int(num1),int(num2)), developer_name = 'Ismail')
    else:
        return render_template("result.html", developer_name = "ismail")

# Add a statement to run the Flask application which can be debugged.
if __name__== "__main__":
    #app.run(debug=True)
    app.run(host='0.0.0.0', port=8082, debug=True)  # Changed port to 8082 for testing purposes